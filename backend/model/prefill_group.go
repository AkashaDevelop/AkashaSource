...
func (g *PrefillGroup) Insert() error {
	...
	g.CreatedTime = now
	g.UpdatedTime = now
	return common.DB.Create(g).Error
}

func (g *PrefillGroup) Update() error {
	g.UpdatedTime = common.GetTimestamp()
	return common.DB.Save(g).Error
}

func DeletePrefillGroupByID(id int) error {
	return common.DB.Delete(&PrefillGroup{}, id).Error
}
...
func IsPrefillGroupNameDuplicated(id int, name string) (bool, error) {
	...
	var cnt int64
	err := common.DB.Model(&PrefillGroup{}).Where("name = ? AND id <> ?", name, id).Count(&cnt).Error
	return cnt > 0, err
}

func GetAllPrefillGroups(groupType string) ([]*PrefillGroup, error) {
	var groups []*PrefillGroup
	query := common.DB.Model(&PrefillGroup{})
	if groupType != "" {
