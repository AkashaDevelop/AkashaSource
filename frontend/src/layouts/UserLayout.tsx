...
export default function UserLayout() {
  ...
  return (
    <div className="flex min-h-screen w-full" style={{ background: 'var(--bg-base)' }}>
      <div className="w-64 flex flex-col flex-shrink-0" style={{ background: 'var(--bg-sidebar)', borderRight: '1px solid var(--border-color)', backdropFilter: 'blur(12px)' }}>
        {/* Logo */}
        ...
        {/* 用户信息 */}
        <div className="mx-4 mb-4 p-3 rounded-2xl cursor-pointer transition-all duration-200"
          ...
          <div style={{ padding: '6px 10px', borderRadius: '8px', background: 'var(--bg-base)', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>余额</span>
            <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--accent-cosmic)' }}>${((user?.quota ?? 0) / 500000).toFixed(4)}</span>
          </div>
        </div>

        {/* 导航 */}
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
          {navItems.map(({ key, icon: Icon, label, external }: any) => {
            const isActive = !external && location.pathname === key;
            return (
              <button key={key}
                onClick={() => external ? window.open(key, '_blank') : navigate(key)}
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200"
                style={isActive ? { background: 'var(--nav-active-bg)', color: 'var(--accent-primary)', boxShadow: 'var(--shadow-card)' } : { color: 'var(--text-secondary)' }}
                onMouseEnter={e => { if (!isActive) { (e.currentTarget as HTMLElement).style.background = 'var(--nav-hover-bg)'; (e.currentTarget as HTMLElement).style.color = 'var(--text-primary)'; } }}
        ...
        <div className="p-4 space-y-2" style={{ borderTop: '1px solid var(--border-color)' }}>
          ...
          {user && user.role >= 10 && (
            <Button variant="flat" startContent={<Shield size={16} />} className="w-full"
              style={{ background: 'rgba(124,58,237,0.12)', color: 'var(--accent-primary)', borderRadius: '12px' }}
              onPress={() => navigate('/admin')}>
              管理后台
            </Button>
          )}
          <Button variant="flat" startContent={<LogOut size={16} />} className="w-full"
            style={{ background: 'rgba(248,113,113,0.12)', color: '#f87171', borderRadius: '12px' }}
