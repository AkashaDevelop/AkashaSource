...
function MJTasksTable({ token }: { token: string }) {
  ...
  const fetchTasks = async (p: number) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/tasks/mj?p=${p}&size=${size}`, {
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)' }}>
        <div className="grid text-xs font-semibold px-4 py-3"
          style={{ gridTemplateColumns: '60px 80px 1fr 100px 80px 160px 90px', color: 'var(--text-secondary)', borderBottom: '1px solid var(--border-color)' }}>
          <span>ID</span>
          <span>操作</span>
          <span>Prompt</span>
          <span>状态</span>
        ...
        {loading ? (
          ...
          tasks.map(task => (
            <div key={task.id}
              ...
              style={{ gridTemplateColumns: '60px 80px 1fr 100px 80px 160px 90px', borderBottom: '1px solid var(--border-color)' }}
              onMouseEnter={e => (e.currentTarget.style.background = 'var(--bg-elevated)')}
              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
            >
              <span className="text-sm font-mono" style={{ color: 'var(--text-secondary)' }}>{task.id}</span>
...
function SunoTasksTable({ token }: { token: string }) {
  ...
  const fetchTasks = async (p: number) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/tasks/suno?p=${p}&size=${size}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (data.code === 0) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)' }}>
        <div className="grid text-xs font-semibold px-4 py-3"
          style={{ gridTemplateColumns: '60px 200px 80px 100px 160px 90px', color: 'var(--text-secondary)', borderBottom: '1px solid var(--border-color)' }}>
          <span>ID</span>
          <span>Task ID</span>
          <span>操作</span>
          <span>状态</span>
        ...
        {loading ? (
          ...
          tasks.map(task => (
            <div key={task.id}
              className="grid items-center px-4 py-3 transition-colors"
              style={{ gridTemplateColumns: '60px 200px 80px 100px 160px 90px', borderBottom: '1px solid var(--border-color)' }}
              onMouseEnter={e => (e.currentTarget.style.background = 'var(--bg-elevated)')}
              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
            >
              <span className="text-sm font-mono" style={{ color: 'var(--text-secondary)' }}>{task.id}</span>
